...
def test_pio():
    ...
    irq(block,1)
    nop()
    wait(0,pin,0).side(0)   #wait for red low. no assert
    wait(1,pin,0).side(0)   #wait for red high. no assert
    wait(0,pin,0).side(2)   #wait for red low.  assert white
    wait(1,pin,0).side(0)   #wait for red high. release assert
    wait(0,pin,0).side(2)   #wait for red low. assert white again
    wait(1,pin,0).side(2)   #wait for red high. keep white assert
...
while True:
    ...
    print(" INS PC EXC RTRY"+str([instr,hex(addr),hex(exctrl),hex(retrycount)]),end="\r")
    time.sleep_ms(200)
    retrycount += 1
'''
print("PIO Disassembly: ")
for data in test_pio[0]:
    exctrl = machine.mem32[PIO0_BASE+SM0_EXECCTRL]
    pictrl = machine.mem32[PIO0_BASE+SM0_PINCTRL]
    instr = decode_pio(data,(pictrl >> 26) & 7,True if exctrl & (1<<30) else False)
    print(instr)
